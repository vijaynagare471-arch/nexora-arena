// =====================================================
// NEXORA ARENA — Backend Server
// Express + SSE Real-time Admin ↔ App Sync
// =====================================================
...
// Load database before handling request
app.use(async (req, res, next) => {
  if (process.env.MONGODB_URI) {
    try {
      if (!mongoClient) {
        ...
        const database = mongoClient.db('nexora');
        dbCollection = database.collection('main');
      }
      
      const doc = await dbCollection.findOne({ _id: 'db_state' });
      if (doc) {
        currentDb = doc.data;
      } else {
      ...
      console.error("Failed to load MongoDB DB:", e);
      try {
  ...
  // Override res.send to wait for all background writes
  ...
  if (process.env.MONGODB_URI) {
    ...
    const writePromise = Promise.all(uploadPromises).then(async () => {
      if (dbCollection) {
        ...
        console.log("Successfully wrote updated DB state to MongoDB.");
      }
    }).catch(err => {
...
async function uploadBase64Field(parentObj, key, base64Str) {
  try {
    ...
    const rawBase64 = matches ? matches[2] : base64Str;
    const apiKey = process.env.IMGBB_API_KEY || '2ef00331005b63bc2e2a0f8ad405ccae';
    ...
    if (res.ok) {
      ...
      if (result.success && result.data && result.data.url) {
        ...
        console.log(`Successfully uploaded base64 to ImgBB: ${publicUrl}`);
      }
    }
  } catch (err) {
    console.error("Failed to upload base64 image to ImgBB:", err);
  }
}
...
function broadcast(event, data) {
  ...
  console.log(`[SSE] Broadcast "${event}" → ${sseClients.length} clients`);
}
...
app.listen(PORT, () => {
  console.log('\n╔══════════════════════════════════════╗');
  console.log('║     NEXORA ARENA SERVER STARTED       ║');
  console.log('╠══════════════════════════════════════╣');
