...
function renderGamesHub() {
  ...
  if (!grid) return;
  const enabled = casinoSettings?.gamesEnabled || {
    plane: true, mines: true, spin: true, dice: true, coinflip: true,
    limbo: true, tower: true, plinko: true, keno: true, color: true, bonus: true
  };
  const logos = casinoSettings?.gamesLogos || {};

  const games = [
    { id: 'plane', name: 'Plane Crash', emoji: '✈️', desc: 'Bet & cash out before crash', bannerBg: 'linear-gradient(135deg,#0d1a3a,#1e3a6e)', label: 'LIVE' },
    { id: 'mines', name: 'Mines', emoji: '💣', desc: 'Avoid mines, collect gems', bannerBg: 'linear-gradient(135deg,#1a0a2e,#3b1f6e)', label: 'HOT' },
    { id: 'spin', name: 'Spin & Win', emoji: '🎡', desc: 'Spin the wheel, win big', bannerBg: 'linear-gradient(135deg,#1a0a1a,#4a1a4a)' },
    { id: 'dice', name: 'Dice', emoji: '🎲', desc: 'Roll high or low, double up', bannerBg: 'linear-gradient(135deg,#0a1a2e,#0d3a6e)' },
    { id: 'coinflip', name: 'Coinflip', emoji: '🪙', desc: 'Heads or tails, double cash', bannerBg: 'linear-gradient(135deg,#1a1a0a,#3a3a0a)' },
    { id: 'limbo', name: 'Limbo', emoji: '📈', desc: 'Target multiplier win multiplier', bannerBg: 'linear-gradient(135deg,#0a1a0a,#0a3a1a)' },
    { id: 'tower', name: 'Tower', emoji: '🗼', desc: 'Climb blocks, increase multiply', bannerBg: 'linear-gradient(135deg,#1a0a0a,#4a1a1a)' },
    { id: 'color', name: 'Color Prediction', emoji: '🎨', desc: 'Red/Green/Violet color win', bannerBg: 'linear-gradient(135deg,#1a0a1a,#3a0a3a)' },
    { id: 'plinko', name: 'Plinko', emoji: '⬇️', desc: 'Drop ball down peg grid', bannerBg: 'linear-gradient(135deg,#0a1a1a,#0a3a3a)' },
    { id: 'keno', name: 'Keno', emoji: '🔢', desc: 'Pick 1-10 numbers, match win', bannerBg: 'linear-gradient(135deg,#1e0a2e,#3b0f5e)' },
    { id: 'bonus', name: 'Daily Bonus Spin', emoji: '🎁', desc: 'Spin daily, free cash reward', bannerBg: 'linear-gradient(135deg,#1a1a0a,#4a3a0a)', label: 'FREE' }
  ];

  const activeGames = games.filter(g => enabled[g.id] !== false);

  grid.innerHTML = activeGames.map(g => {
    const labelHTML = g.label ? `<span class="game-hub-banner-label" style="${g.label === 'FREE' ? 'background:#10b981' : ''}">${g.label}</span>` : '';
    
    // Custom logo image check
    let mediaHTML = g.emoji;
    if (logos[g.id]) {
      mediaHTML = `<div style="width:45px; height:45px; border-radius:10px; background-image:url(${logos[g.id]}); background-size:cover; background-position:center; box-shadow: 0 4px 10px rgba(0,0,0,0.35);"></div>`;
    }

    return `
      <div class="game-hub-card" onclick="openCasinoGame('${g.id}')">
        <div class="game-hub-banner" style="background:${g.bannerBg};">
          ${mediaHTML}
          ${labelHTML}
        </div>
        ...
        <button class="game-hub-play-btn">PLAY NOW</button>
      </div>
    `;
  }).join('');
}
